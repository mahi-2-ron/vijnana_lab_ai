...
const Home: React.FC = () => {
  ...
  return (
    <div className="w-full min-h-screen pt-24 pb-12 relative overflow-hidden">
      ...
      {/* Quick Tools Section */}
      <section className="px-6 md:px-12 lg:px-20 pb-12 z-20 relative isolate">
        <StudentToolkit />
      </section>
      ...
      <section className="px-6 md:px-12 lg:px-20 py-16 z-10 relative">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          <div className="max-w-2xl">
            <h2 className="text-3xl md:text-4xl font-display font-bold text-slate-900 dark:text-white mb-4 transition-colors">
              Theory <span className="text-blue-500">Beyond</span> the Lab
            </h2>
            <p className="text-lg text-blue-600 dark:text-blue-300 italic font-medium">
              “Not just experiments — we help students understand the theory behind them.”
            </p>
          </div>
          <Link to="/blog">
            <button className="px-6 py-3 rounded-full bg-white/5 border border-white/10 text-slate-900 dark:text-white font-bold hover:bg-white/10 transition-all flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
           {[...Array(3)].map((_, i) => (
             <Link key={i} to={`/blog/b${i+1}`}>
               <GlassCard className="h-full group border-white/5 hover:border-blue-500/30 transition-all" color="blue">
                  <div className="flex justify-between items-center mb-4">
                     <span className="px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-500 text-[10px] font-bold uppercase tracking-wider">Featured</span>
                     <span className="text-[10px] text-gray-500 font-medium">Concept Guide</span>
                  </div>
                  <h3 className="text-xl font-bold text-white mb-3 group-hover:text-blue-400 transition-colors">
                     {i === 0 ? "Ohm's Law: Traffic Control" : i === 1 ? "Titration: Chemical Balance" : "Cell Division: Copy-Paste"}
                  </h3>
                  <p className="text-sm text-gray-400 line-clamp-2 mb-6">
                     {i === 0 ? "Understand voltage and current with our simple pipe analogy." : i === 1 ? "Why does the color change suddenly? Learn the secret." : "How one cell becomes two identical twins."}
                  </p>
